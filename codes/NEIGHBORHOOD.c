...
int BW, SW, SWR, ran_i, ran_h, fil, col, Sfil, Scol, f[NN], c[NN], Vk;
...
FILE *fptr, *frep, *fgen, *ffmap, *ffmapR, *ffped, *ffpedR;
...
plink_files()
{
	...
	/* **************** ped file with 13 dispersed SWR=15 - dataR.ped ***************** */

	// ****** Upper left corner ******* */

	for (Sfil=0; Sfil<SWR; Sfil++)
	for (Scol=0; Scol<SWR; Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
	...
	for (Sfil=0; Sfil<SWR; Sfil++)
	for (Scol=((sqrt(NIND)/2)-(SWR/2)); Scol<=(sqrt(NIND)/2)+(SWR/2); Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
		...
		fprintf(ffpedR,"1 INDML%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	for (Sfil=0; Sfil<SWR; Sfil++)
	for (Scol=(sqrt(NIND)-SWR); Scol<sqrt(NIND); Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
	...
	for (Sfil=((sqrt(NIND)/2)-(SWR/2)); Sfil<=(sqrt(NIND)/2)+(SWR/2); Sfil++)
	for (Scol=0; Scol<SWR; Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
		...
		fprintf(ffpedR,"1 INDUC%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	for (Sfil=(sqrt(NIND)-SWR); Sfil<sqrt(NIND); Sfil++)
	for (Scol=0; Scol<SWR; Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
	...
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		...
		fprintf(ffpedR,"1 INDMR%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		...
		fprintf(ffpedR,"1 INDLC%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	for (Sfil=((sqrt(NIND)/4)-(SWR/2)); Sfil<=(sqrt(NIND)/4)+(SWR/2); Sfil++)
	for (Scol=((sqrt(NIND)/4)-(SWR/2)); Scol<=(sqrt(NIND)/4)+(SWR/2); Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
		...
		fprintf(ffpedR,"1 INDMUL%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	for (Sfil=((sqrt(NIND)/4)-(SWR/2)); Sfil<=(sqrt(NIND)/4)+(SWR/2); Sfil++)
	for (Scol=((3*sqrt(NIND)/4)-(SWR/2)); Scol<=(3*sqrt(NIND)/4)+(SWR/2); Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
		...
		fprintf(ffpedR,"1 INDMUR%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	for (Sfil=((3*sqrt(NIND)/4)-(SWR/2)); Sfil<=(3*sqrt(NIND)/4)+(SWR/2); Sfil++)
	for (Scol=((sqrt(NIND)/4)-(SWR/2)); Scol<=(sqrt(NIND)/4)+(SWR/2); Scol++)
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		i = (Sfil*sqrt(NIND)) + Scol;
		...
		fprintf(ffpedR,"1 INDMLL%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
	...
	if ((Sfil >= 0) && (Scol >= 0) && (Sfil < sqrt(NIND)) && (Scol < sqrt(NIND)))
	{
		...
		fprintf(ffpedR,"1 INDMLR%d 0 0 1 -9 ", i);
		for (k=0; k<NCRO; k++)
		for (l=0; l<NLOCI; l++)
		if ((q[k][l] != 0.0)&&(q[k][l] != 1.0))
		{
...
mating ()
{
	...
	if (tracelevel!=0)	if (gen == 1000)	fprintf(fptr,"\n Parents \n");
...
//			if (tracelevel!=0)	fprintf(fptr,"i = %d    rndfil = %d    rnd col = %d   p1 = %d \n", i, rndfil, rndcol, p1);
		...
		if (tracelevel!=0)   if(gen == 1000)	fprintf (fptr,"%d\t%d\n", p1, p2);
